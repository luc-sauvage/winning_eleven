    ...
    useEffect(() => {
        ...
        axios.get(
            "https://v3.football.api-sports.io/fixtures/rounds?league=135&season=2020&current=true",
            {
                ...
                console.log("response for matchDay", matchDayResponse.data.response[0]);
                const matchDayArray = (matchDayResponse.data.response[0]).match(/(\d+)/);
                ...
                axios.get(`/stats/${fetchedMatchDay}`).then((serverResponse) => {
