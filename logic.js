function logic(playersInfos, matchDay) {
    ...
    function commonLogic(statistics) {
        return (
            statistics.rating * weight.rating +
                ...
                (statistics.missed_penalties + statistics.scored_penalties > 0
                    ? (statistics.missed_penalties /
                          (statistics.missed_penalties +
                              statistics.scored_penalties)) *
                      weight.averageMissedPenalties
                    : 0) +
                (statistics.total_interceptions / statistics.match_day) *
                    weight.averageInterceptions +
                (statistics.won_duels / statistics.total_duels) *
                    weight.wonDuelsRate ||
            0 +
                (statistics.key_passes / statistics.match_day) *
                    weight.averageKeyPasses +
                (statistics.drawn_fouls / statistics.match_day) *
                    weight.averageDrawnFouls +
                (statistics.commited_penalties / statistics.match_day) *
                    weight.averageCommittedPenalties +
                (statistics.won_penalties / statistics.match_day) *
                    weight.averageWonPenalties
        );
    }
    ...
    function calculateFinalPoints(statistics) {
        ...
        if (statistics.position === "Goalkeeper") {
            weightedResult =
                statistics.rating * weight.rating +
                (statistics.conceded_goals / statistics.match_day) *
                    weight.averageConcededGoals +
                (statistics.yellow_cards / statistics.match_day) *
                    weight.averageYellowCards +
                (statistics.red_cards / statistics.match_day) *
                    weight.averageRedCards +
                (statistics.lineups / statistics.match_day) *
                    weight.lineupLikelyhood +
                (statistics.saved_penalties / statistics.match_day) *
                    weight.averageSavedPenalties +
                (statistics.saves / statistics.match_day) *
                    weight.averageSaves +
                (statistics.commited_penalties / statistics.match_day) *
                    weight.averageCommittedPenalties;
        } else if (statistics.position === "Defender") {
            weightedResult =
                commonLogic(statistics) +
                (statistics.committed_fouls / statistics.match_day) *
                    weight.averageFouls +
                (statistics.total_tackles / statistics.match_day) *
                    weight.averageTackles;
            ...
            weightedResult =
                ...
                (statistics.total_tackles / statistics.match_day) *
                    weight.averageTackles +
                (statistics.total_passes / statistics.match_day) *
                    weight.averagePasses;
        } else {
